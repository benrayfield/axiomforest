...
public class MathUtil{
	...
	*/
	...
	public static long[] bytesToLongs(byte[] b){
		if((b.length&7)!=0) throw new RuntimeException("Hash output is not a multiple of 8 bytes: "+b.length);
		long[] ret = new long[b.length>>3];
		for(int i=0; i<ret.length; i++){ //copy hashBytes to hashLongs as bigEndian
			ret[i] = readLongFromByteArray(b, i<<3);
		}
		return ret;
	}
